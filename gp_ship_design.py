...
####################################
# Crane modules indexed by i
I = [0,1,2]
# Engine modules indexed by j
J = [0,1,2]
####################################
# endregion
...
P_C = [20,50,100]
# Performance of engine module j
P_E = [20,30,40]
# Cost of crane module i
C_C = [2,4.5,8]
# Cost of engine module j
C_E = [3,5.5,9]
# Goal Lifting capacity [ton]
G_L = 70
# Goal ship speed [knot = 1.852 km/h]
G_S = 30
# Maximum Aquisition cost [$]
M_C = 12
####################################
# endregion
...
# Positive and Negative deviation between Goal Lifting capacity and Lifting capacity of crane module
D_positive_lifting = pl.LpVariable('d_positive_lifting', lowBound= 0, upBound = None, cat = pl.LpContinuous)
D_negative_lifting = pl.LpVariable('d_negative_lifting', lowBound= 0, upBound = None, cat = pl.LpContinuous)

# Positive and Negative deviation between Goal Ship speed and Ship speed of engine module
...
####################################
objective_function = D_positive_lifting + D_negative_lifting + D_positive_speed + D_negative_speed
...
####################################
# 1). constraint : Make a relationship between x and D_lift and G_L
temp = []
for i in I:
    temp.append((X[i], P_C[i]))
const1 = pl.LpAffineExpression(temp) + D_negative_lifting - D_positive_lifting
model += const1 <= G_L

# 2). constraint : Make a relationship between y and D_speed and G_S
temp = []
for j in J:
...
# 3). constraint : Total aquisiton cost have upper limit
temp1 = []
temp2 = []
for i in I:
...
print("-------------------------------------------------------------")
print("Deviation negative of Lifting_Capacity: ", pl.value(D_negative_lifting))
print("Deviation positive of Lifting_Capacity: ", pl.value(D_positive_lifting))
